...
hvec4 fog_process(vec3 vertex) {
	...
	if (sc_use_fog_aerial_perspective()) {
		...
		sky_fog_color = textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(cube_view, lod)).rgb;
		sky_fog_color = mix(sky_fog_color, textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(cube_view, lod + 1)).rgb, blend);
#else
...
// If alpha scissor is used, we must further the edge threshold, otherwise we won't get any edge feather
	...
	if (sc_scene_use_reflection_cubemap()) {
		...
		hvec3 indirect_sample_a = hvec3(textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(vec3(ref_vec), float(lod))).rgb);
		hvec3 indirect_sample_b = hvec3(textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(vec3(ref_vec), float(lod) + 1.0)).rgb);
...
#ifdef USE_RADIANCE_CUBEMAP_ARRAY
			hvec3 cubemap_ambient = hvec3(textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(ambient_dir, MAX_ROUGHNESS_LOD)).rgb);
#else
	...
	if (sc_scene_use_reflection_cubemap()) {
		...
		hvec3 clearcoat_sample_a = hvec3(textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(ref_vec, lod)).rgb);
		hvec3 clearcoat_sample_b = hvec3(textureFix(samplerCubeArrayFix(radiance_cubemap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec4(ref_vec, lod + 1)).rgb);
