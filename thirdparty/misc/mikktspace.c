...
#ifdef ARCH_ARM64
    return vld1q_f32(&v_copy.x);
#elif ARCH_SSE
    return _mm_loadu_ps(&v_copy.x);
#else
    SVec3 r;
    r.x = v_copy.x;
    r.y = v_copy.y;
...
{
#ifdef ARCH_ARM64
    SVec3 r = vmulq_n_f32(v, fS);
    // Set the w component to 0.
    r = vsetq_lane_f32(0.0f, r, 3);
#elif ARCH_SSE
    SVec3 r = _mm_mul_ps(v, _mm_set_ps1(fS));
    // Set the w component to 0.
    SVec3 mask = _mm_castsi128_ps(_mm_set_epi32(0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x00000000));
    r = _mm_and_ps(r, mask);
#else
    SVec3 r;
    r.x = v.x * fS;
...
{
#ifdef ARCH_ARM64
	SVec3 r = vdivq_f32(v1, v2);
    // Set the w component to 0.
    r = vsetq_lane_f32(0.0f, r, 3);
#elif ARCH_SSE
    SVec3 r = _mm_div_ps(v1, v2);
    // Set the w component to 0.
    SVec3 mask = _mm_castsi128_ps(_mm_set_epi32(0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x00000000));
    r = _mm_and_ps(r, mask);
#else
...
static SVec3 GetPosition(const SMikkTSpaceContext * pContext, const int index)
{
	...
	res.x = p[0]; res.y = p[1]; res.z = p[2]; res.w = 0.0f;
	return vmake(res);
}
...
static SVec3 GetTexCoord(const SMikkTSpaceContext * pContext, const int index)
{
	...
	res.x = p[0]; res.y = p[1]; res.z = 1.0f; res.w = 0.0f;
	return vmake(res);
}
