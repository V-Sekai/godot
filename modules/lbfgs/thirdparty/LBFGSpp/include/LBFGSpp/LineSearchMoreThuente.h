...
public:
    ...
    static int LineSearch(Foo& f, const SolverParam& param,
                           ...
                           Scalar& step, Scalar& fx, Vector& grad, Scalar& dg, Vector& x)
    {
        ...
        if (step > step_max) {
            std::cerr << "'step' exceeds 'step_max'" << std::endl;
            return -2;
        }
        ...
        if (dg_init >= Scalar(0)) {
            std::cerr << "the moving direction does not decrease the objective function value" << std::endl;
            return -3;
        }
        ...
        for (iter = 0; iter < param.max_linesearch; iter++)
        {
            ...
            if (step < param.min_step) {
                std::cerr << "The line search step became smaller than the minimum value allowed." << std::endl;
                return ERR_INVALID_PARAMETER;
            }

            if (step > param.max_step) {
                std::cerr << "The line search step became larger than the maximum value allowed." << std::endl;
                return ERR_INVALID_PARAMETER;
            }
