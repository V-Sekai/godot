...
def get_godot_member_url(class_name, member_name, member_type):
    ...
    return (
        get_godot_class_url(class_name)
        + "#class-"
        + class_name.lower()
        + "-"
        + member_type
...
def class_doc_to_markdown(
    ...
    if len(klass.properties) > 0:
        ...
        for prop in klass.properties:
            row = [fmt.make_type(prop.type), make_custom_internal_link(prop.name)]
    ...
    if len(klass.methods) > 0:
        ...
        for method in klass.methods:
            ...
            row = [fmt.make_type(method.return_type), signature]
    ...
    if len(klass.methods) > 0:
        ...
        for method in klass.methods:
            out += (
                ...
                + make_custom_internal_anchor(method.name)
                + " **"
                + method.name
                + "**"
            )
