...
def register_scons_options(env, is_extension):
    ...
    env_vars.Add(
        BoolVariable(
            "voxel_tests", "Build with tests for the voxel module, which will run on startup of the engine", False
        )
    )
    ...
    if not is_extension:
        ...
        env_vars.Add(BoolVariable("voxel_werror", "Explicitly enable warnings as errors for module code only", False))
...
def get_sources(env, is_editor_build):
    ...
    if smoosh_meshing_enabled:
        ...
        if gpu_enabled:
            ...
            if tests_enabled:
                sources += ["tests/voxel/test_detail_rendering_gpu.cpp", "tests/voxel/test_transvoxel.cpp"]
