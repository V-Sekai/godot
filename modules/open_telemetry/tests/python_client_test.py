...
def setup_tracing(service_name="python-test-client", endpoint="http://localhost:4318/v1/traces"):
    ...
    resource = Resource.create({"service.name": service_name, "service.version": "1.0.0", "host.name": "test-machine"})
...
def test_metrics(meter):
    ...
    # Create counter
    counter = meter.create_counter("requests_total", description="Total number of requests", unit="1")
    counter.add(100, {"method": "GET", "status": "200"})

    # Create histogram
    histogram = meter.create_histogram("request_duration", description="Request duration", unit="ms")
...
def export_to_file(provider, filename):
    ...
    print("✓ Data exported (check OTLP endpoint or console output)")
    print("  To use in Godot:")
    print("  var reflector = OTelReflector.new()")
    print("  # Configure OTLP endpoint to receive this data")
