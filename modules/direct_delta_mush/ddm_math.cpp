...
void eigen_decomposition_3x3(
    ...
    for (int iter = 0; iter < 10; iter++) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                float val = A.m[i][j];
                if (i == 0) Av.x += val * v.x;
                else if (i == 1) Av.y += val * v.x;
                else Av.z += val * v.x;
                
                if (i == 0) Av.x += val * v.y;
                else if (i == 1) Av.y += val * v.y;
                else Av.z += val * v.y;
                
                if (i == 0) Av.x += val * v.z;
                else if (i == 1) Av.y += val * v.z;
                else Av.z += val * v.z;
            }
        }
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            float val = A.m[i][j];
            if (i == 0) Av.x += val * v.x + val * v.y + val * v.z;
            else if (i == 1) Av.y += val * v.x + val * v.y + val * v.z;
            else Av.z += val * v.x + val * v.y + val * v.z;
        }
    }
...
Vector3 compute_displacement(
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (i == 0) transformed.x += M_scale_shear.m[i][j] * rest_vertex.x;
            else if (i == 1) transformed.y += M_scale_shear.m[i][j] * rest_vertex.y;
            else transformed.z += M_scale_shear.m[i][j] * rest_vertex.z;
        }
    }
...
Vector3 enhanced_deformation(
    ...
    for (uint32_t b = 0; b < max_bones && b < bone_weights.size(); b++) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (i == 0) transformed.x += M_rigid.m[i][j] * displaced.x;
                else if (i == 1) transformed.y += M_rigid.m[i][j] * displaced.y;
                else transformed.z += M_rigid.m[i][j] * displaced.z;
            }
        }
...
Vector3 standard_deformation(
    ...
    for (uint32_t b = 0; b < max_bones && b < bone_weights.size(); b++) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (i == 0) transformed.x += M.m[i][j] * smoothed_vertex.x;
                else if (i == 1) transformed.y += M.m[i][j] * smoothed_vertex.y;
                else transformed.z += M.m[i][j] * smoothed_vertex.z;
            }
        }
