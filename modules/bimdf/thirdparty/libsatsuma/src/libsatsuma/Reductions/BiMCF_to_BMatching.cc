...
namespace Satsuma {
        ...
        : bimcf_(_bimcf)
    {
        ...
        for (const auto bimcf_node: _bimcf.g.nodes())
        {
            ...
            if (demand != 0) {
                // Handle non-zero demands without throwing an exception
                demand = 0; // Set demand to zero or handle it appropriately
            }
            if (demand > 0) {
...
BiMCFResult BiMCF_to_BMatching::translate_solution(const BMatchingResult &bmatching_result) const
{
    ...
    if (!bimcf_.is_valid(*sol)) {
        // Handle invalid solution without throwing an exception
        BiMCFResult result;
