...
(*
  ...
  3. need1 tries m_need1: [getv(1)] -> fails (flag=0, not 1) -> blacklist m0, reopen put_it
  4. put_it tries m1: [putv(1), getv(1)] -> should succeed, flag=1
  5. need1 tries m_need1: [getv(1)] -> should succeed (flag=1)
*)
...
TypeOK ==
    ...
    /\ DOMAIN solutionGraph \subseteq (0..MaxNodes)
    /\ \A nodeId \in DOMAIN solutionGraph:
        solutionGraph[nodeId] \in [status: {"OPEN", "CLOSED", "FAILED"},
...
(* Check if command array is blacklisted *)
IsBlacklisted(cmd) ==
...
Next ==
    \/ \E nodeId \in 0..MaxNodes:
         /\ nodeId \in DOMAIN solutionGraph
