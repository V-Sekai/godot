...
def compare_program(testname, program):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        result = subprocess.run([tmp.name], stdout=subprocess.PIPE)

        native = result.stdout.decode("utf-8")

    with tempfile.NamedTemporaryFile() as tmp:
        result = subprocess.run(
            [RISCV_COMPILER, "-x", "c", "-static", "-o", tmp.name, "-"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            input=program,
        )

        result = subprocess.run(["emulator/build/emulator", tmp.name], stdout=subprocess.PIPE)
